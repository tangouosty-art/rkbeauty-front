...
async function loadSessionsForFormationCode(code) {
  ...
  if (!code) {
    sessionSel.innerHTML = `<option value="">— Sélectionnez une formation d’abord —</option>`;
    return;
  }
  ...
  if (!Array.isArray(sessions) || sessions.length === 0) {
    sessionSel.innerHTML = `<option value="">Aucune session disponible</option>`;
    setHelp("La session de cette formation n’est pas encore disponible. Sélectionnez une autre formation.", false);
    return;
  }

  // on affiche start_date + remaining
  sessionSel.innerHTML =
    `<option value="">— Choisir une date —</option>` +
    sessions.map(s => {
      const label = `${fmtDateFR(s.start_date)} (${s.remaining} place(s) restante(s))`;
      return `<option value="${s.id}" data-start="${s.start_date}">${label}</option>`;
    }).join("");
...
formationSelect?.addEventListener("change", async () => {
  try {
    const code = formationSelect.value; // maintenant value = F2J-150 etc.
    await loadSessionsForFormationCode(code);
  } catch (e) {
    console.error(e);
...
function resetSlotUI() {
  ...
  if (morningOpt) {
    ...
    morningOpt.textContent = "Matin de 11h30 à 14h30";
  }
  if (afternoonOpt) {
    afternoonOpt.disabled = false;
    afternoonOpt.textContent = "Après-midi de 15h30 à 18h30";
  }
...
function setOptionState(value, disabled, labelSuffix = "") {
  ...
  opt.textContent =
    value === "morning"
      ? `Matin de 11h30 à 14h30${labelSuffix}`
      : `Après-midi de 15h30 à 18h30${labelSuffix}`;
}
...
const payload = {
  ...
  slot: slotValue, // morning/afternoon comme avant
  customer: { name, email, phone },
  formation: formationLabel,        // ex: "Formation 2 jours - 150€"
  totalPriceEUR: totalPriceValue,   // comme avant
